...
Creating a class for storing vectors' X and Y coordinates and modify it so that these vectors can add-up with each other and print them in a customized way.
Nearest drives -> create an efficient way to find 'n' nearest drivers to a passenger (hint: an optimized way is using a min-heap)
...
- Maktabkhone -> Design signals based on the observer design pattern (signal register + triggering)
