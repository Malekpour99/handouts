...
- `:[start],[end]d`: Deletes lines _start_ through _end_
...
> All of this search and navigation can be combined with yank, change or delete actions!

- `%`: Jumps between matching parentheses or brackets
- `t<character>`: Searches for the character's next occurrence on the current line and jumps **right before** that character
- `<S-t><character>`: Searches for the character's previous occurrence on the current line and jumps **right after** that character
- `f<character>`: Searches for the character's next occurrence on the current line and jumps **right on that** character
- `<S-f><character>`: Searches for the character's previous occurrence on the current line and jumps **right on that** character
